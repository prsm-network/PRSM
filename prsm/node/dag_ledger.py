...
class DAGLedger(LedgerNodeServicesMixin):
    ...
    async def _create_tables(self) -> None:
        ...
        await self._db.executescript("""
            ...
            -- These match the LocalLedger schema so DAGLedger carries the same node-services
            -- surface (gossip + provenance + collaboration + allowances) via
            -- LedgerNodeServicesMixin — sp966/975/1425.
    ...
    async def get_recent_tx_ids(self, wallet_id: str, limit: int = 50) -> List[str]:
        ...
        Sprint 1419. This existed on LocalLedger but NOT here — and `Node._initialize()` builds a
        bare DAGLedger whenever `config.ledger_type == "dag"`, which is the DEFAULT. (A dead
        DAGLedgerAdapter also had it and made it look covered; that adapter was removed in sp1427.)
        So on every

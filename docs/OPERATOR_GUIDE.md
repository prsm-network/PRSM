...
## Becoming a Production Compute Operator

The progression from a bare node to a paid, production-trust GPU operator that serves models too big
for one card. Each level builds on the last; stop at whichever fits your hardware and goals.

### Level 1 — Serve inference (any node)

Install and start — the node serves a local model out of the box, no env ceremony:
```bash
pip install -e '.[ml]'
prsm node start
prsm node inference-status      # READY once the model is loaded
```

### Level 2 — Serve a real model on GPU

A GPU node serves a large HuggingFace model; the node auto-configures it — deps are pinned, the
catalog is auto-derived from the model's config, and the model registry is auto-staged at start
(sp1375–1377), so no manual archaeology:
```bash
...
prsm node start
```
To shard a model too big for one card across multiple GPU nodes, follow the
**[big-model GPU operator guide](2026-07-05-big-model-gpu-operator-guide.md)** (static pool + anchor
generated by `scripts/gen_cluster_config.py`, cross-node routing, SSH-tunnel bridging for restricted
networks).

### Level 3 — Get paid (on-chain settlement)

Provision a funded settler key + wallet map, run the go-live preflight, and settle inference on-chain.
Full ceremony (single-payee and per-node multi-stage) is in the
**[multi-stage settlement guide](2026-07-04-multistage-settlement-gpu-canary-plan.md)**; gate every
live run with `python -m prsm.settlement.go_live_preflight`.

### Level 4 — Production trust (real slashable stake)

`mock` trust accepts any anchored peer; **production** trust requires on-chain stake. Post it in one
...
```
Exit later: `prsm node stake-unbond` → wait the unbond delay → `prsm node stake-withdraw`.

### Level 5 — Monitor your stake

Stake is **slashable**, so watch for challenges (before they cost you) and slashes:
```bash
prsm node challenge-status     # challenges against your committed batches (EARLY warning)
prsm node slash-status         # Slashed penalties (the resolved outcome)
```
Or run it continuously: set `PRSM_SETTLEMENT_CHALLENGE_WATCHER_ENABLED=1` and the node logs a WARNING
on each new challenge and exposes the `prsm_settlement_challenges_active` metric for AlertManager (see
**Monitoring** below).
...
| Command | Purpose |
|---|---|
| `prsm node start` | Run the node |
| `prsm node inference-status` | Is the model loaded + serving? |
| `prsm node stake-bond <ftns> [--tier-slash-rate-bps N]` | Post StakeBond stake (approve + bond) |
| `prsm node stake-info` | Current stake + tier |
| `prsm node stake-unbond` / `stake-withdraw` | Exit stake (subject to the unbond delay) |
| `prsm node challenge-status [ADDR]` | Challenges against your committed batches |
| `prsm node slash-status [ADDR]` | Slash penalties against your stake |

Keys always live in your shell environment (`FTNS_WALLET_PRIVATE_KEY`), never on the command line or
in a file the node reads back to callers.

...
def test_unsupported_reason_rejected_by_dry_run_and_submit():
    """(c) CONSENSUS_MISMATCH (reason 5) is NOT in the generic supported set → BOTH dry_run
    and submit reject it uniformly, never raise, never reach the contract / broadcast.

    (Sprint 1147 added NO_ESCROW=2 and sprint 1148 added EXPIRED=3 to the supported set, so
    CONSENSUS_MISMATCH=5 — which has its OWN consensus_submitter because its auxData is
    complex — is now the unsupported-reason exemplar for the generic submitter. The rejection
    contract is unchanged: any reason outside SUPPORTED_CHALLENGE_REASONS is uniformly
    refused.)"""
    h = _Harness(tx_status=1)
    sub = h.submitter()
    bogus = _double_spend_challenge(reason_code=5)  # CONSENSUS_MISMATCH — unsupported here

    dr = sub.dry_run(bogus)
    assert dr.would_succeed is False
    assert dr.revert_reason and "5" in dr.revert_reason
